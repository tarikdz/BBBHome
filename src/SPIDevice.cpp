 ...
 * @param busNumber The SPI bus number X (first digit after spidevX.Y)
 * @param device The device on the bus Y (second digit after spidevX.Y)
 */
SPIDevice::SPIDevice(unsigned int busNumber, unsigned int device, unsigned int gpioNumber):
	BusDevice(busNumber,device) {
	stringstream s;
	s << SPI_PATH << busNumber << "." << device;
	...
	this->gpioAsChipSelect = new GPIO(gpioNumber);
	this->gpioAsChipSelect->setDirection(GPIO::OUTPUT);
	this->gpioAsChipSelect->setValue(GPIO::HIGH);
...
/**
 * GPIO as high - slave not selected (mode 0)
 * GPIO as low - slave active
 */
void SPIDevice::setChipSelect(GPIO::VALUE value){
	this->gpioAsChipSelect->setValue(value);
}
