...
namespace TestARM{
	...
	public:
		SPIDevice(unsigned int busNumber, unsigned int device, unsigned int gpioNumber);
		virtual int open();
		...
		virtual void setChipSelect(GPIO::VALUE value);
	...
	private:
		...
		GPIO *gpioAsChipSelect;  //GPIO that acts as a chip select for SPI(we will have multiple slaves)
